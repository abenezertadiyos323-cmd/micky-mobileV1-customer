...
export function useHotLeads(limit: number = 10) {
  const actions = useQuery(api.phoneActions.listAllPhoneActions);
  const exchanges = useQuery(api.phoneActions.listAllExchangeRequests);
...
function formatLead(lead: any, score: number, type: 'exchange' | 'action'): HotLead {
