...
Micky Mobile — a monorepo powering two Telegram Mini Apps (Customer and Admin) and a shared Convex backend. This file is the root-level single source of truth for Claude Code work; detailed frontend specs live in `micky-mobile-admin-miniapp/FRONTEND_SPEC.md` in the admin package.
...
> Note: Root `claude.md` is the single source of truth for project structure, stack, and constraints. Detailed UI/UX for admin lives in `micky-mobile-admin-miniapp/FRONTEND_SPEC.md`.
